/*
Reccito API

 # Reccito API  A receipt management platform that enables businesses to create, manage, and share digital receipts with customers through QR codes, mobile apps, and web interfaces.  ## Key Features  ### 🏢 **Multi-Tenant Architecture** - Organization and store hierarchy - Scoped API access and permissions - Isolated data per organization  ### 📱 **Digital Receipt Management** - Create and store digital receipts - Generate QR codes for easy customer access - Barcode generation for physical receipts - Time-limited QR codes for security  ### 🔐 **Authentication & Security** - API key authentication for merchants - JWT token authentication for customers - Phone-based customer verification with SMS - Rate limiting per organization - Permission-based access control  ## Authentication  ### Merchant Authentication (API Keys) ``` Authorization: Bearer rcp_live_1234567890abcdef ```  Merchants use API keys to access business endpoints. API keys can be scoped to: - **Organization level**: Access to all stores within organization - **Store level**: Access limited to specific store  ## Rate Limits  API requests are rate limited per organization (defaults: 60/minute, 1,000/hour, 10,000/day; configurable per organization). Responses include rate limit headers: - `X-RateLimit-Limit-Minute` / `X-RateLimit-Remaining-Minute` / `X-RateLimit-Reset-Minute` - `X-RateLimit-Limit-Hour` / `X-RateLimit-Remaining-Hour` / `X-RateLimit-Reset-Hour`  ## Error Handling  The API uses standard HTTP status codes. Error responses use one of two shapes depending on the endpoint — integrators should handle both:  - `404` and `500` responses use a structured envelope:   ```json   {       \"error\": {           \"code\": \"NOT_FOUND\",           \"message\": \"The requested resource was not found\",           \"path\": \"/api/v1/merchant/receipts/abc\"       }   }   ``` - All other error responses (400, 401, 403, 422, etc.) use FastAPI's default   `{\"detail\": ...}` shape, where `detail` may be a plain string or a   structured object depending on the endpoint. For example:   ```json   { \"detail\": \"No active subscription found.\" }   ```   or   ```json   { \"detail\": { \"code\": \"free_tier_quota_exceeded\", \"message\": \"...\" } }   ```   `422` validation errors follow FastAPI's standard `detail: [...]` array   shape (field, message, error type per invalid field).  ## Pagination  List endpoints use offset pagination (`page`/`limit` query parameters):  ```json {     \"receipts\": [...],     \"total\": 150,     \"page\": 1,     \"limit\": 25,     \"has_next\": true,     \"has_prev\": false } ```  ## Data Formats  ### Dates and Times Timestamps are ISO 8601. Most are UTC with an explicit offset (`2024-01-15T14:30:00+00:00`); a few older fields may omit the offset — treat any timestamp without one as UTC.  ### Currency and Amounts Monetary amounts are serialized as **decimal strings** (not numbers), with a separate `currency` field. This avoids floating-point precision loss and keeps every money field consistent across the API: ```json {     \"total_amount\": \"12.99\",     \"currency\": \"GBP\" } ``` Request bodies still lenient-accept a JSON number for amounts, but the published contract and every response are string-only.  ### ULIDs Receipt IDs use ULID format for better performance and sortability: ```json \"01HZXYZ123456789ABCD\" ```  ## Support  - **Support**: [dev-support@reccito.com](mailto:dev-support@reccito.com) - **Interactive docs**: [https://api.reccito.com/api/v1/redoc](https://api.reccito.com/api/v1/redoc)    This document covers Reccito's public integrations API. Admin and account-internal operations are not included. 

API version: 1.0.0
Contact: dev-support@reccito.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package reccitosdk

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the ReceiptImmediateResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ReceiptImmediateResponse{}

// ReceiptImmediateResponse Immediate receipt response for fast QR code generation.
type ReceiptImmediateResponse struct {
	Id string `json:"id"`
	PublicToken string `json:"public_token"`
	Barcode NullableBarcode `json:"barcode,omitempty"`
	MerchantReceiptId NullableString `json:"merchant_receipt_id,omitempty"`
	StoreId string `json:"store_id"`
	OrganisationId string `json:"organisation_id"`
	QrCodeUrl string `json:"qr_code_url"`
	QrToken string `json:"qr_token"`
	QrExpiresAt NullableTime `json:"qr_expires_at"`
	ReceiptUrl string `json:"receipt_url"`
	TransactionDate time.Time `json:"transaction_date"`
	TotalAmount string `json:"total_amount"`
	Currency string `json:"currency"`
	ProcessingStatus *string `json:"processing_status,omitempty"`
	CreatedAt time.Time `json:"created_at"`
}

type _ReceiptImmediateResponse ReceiptImmediateResponse

// NewReceiptImmediateResponse instantiates a new ReceiptImmediateResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReceiptImmediateResponse(id string, publicToken string, storeId string, organisationId string, qrCodeUrl string, qrToken string, qrExpiresAt NullableTime, receiptUrl string, transactionDate time.Time, totalAmount string, currency string, createdAt time.Time) *ReceiptImmediateResponse {
	this := ReceiptImmediateResponse{}
	this.Id = id
	this.PublicToken = publicToken
	this.StoreId = storeId
	this.OrganisationId = organisationId
	this.QrCodeUrl = qrCodeUrl
	this.QrToken = qrToken
	this.QrExpiresAt = qrExpiresAt
	this.ReceiptUrl = receiptUrl
	this.TransactionDate = transactionDate
	this.TotalAmount = totalAmount
	this.Currency = currency
	var processingStatus string = "processing"
	this.ProcessingStatus = &processingStatus
	this.CreatedAt = createdAt
	return &this
}

// NewReceiptImmediateResponseWithDefaults instantiates a new ReceiptImmediateResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReceiptImmediateResponseWithDefaults() *ReceiptImmediateResponse {
	this := ReceiptImmediateResponse{}
	var processingStatus string = "processing"
	this.ProcessingStatus = &processingStatus
	return &this
}

// GetId returns the Id field value
func (o *ReceiptImmediateResponse) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *ReceiptImmediateResponse) SetId(v string) {
	o.Id = v
}

// GetPublicToken returns the PublicToken field value
func (o *ReceiptImmediateResponse) GetPublicToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PublicToken
}

// GetPublicTokenOk returns a tuple with the PublicToken field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetPublicTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PublicToken, true
}

// SetPublicToken sets field value
func (o *ReceiptImmediateResponse) SetPublicToken(v string) {
	o.PublicToken = v
}

// GetBarcode returns the Barcode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ReceiptImmediateResponse) GetBarcode() Barcode {
	if o == nil || IsNil(o.Barcode.Get()) {
		var ret Barcode
		return ret
	}
	return *o.Barcode.Get()
}

// GetBarcodeOk returns a tuple with the Barcode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReceiptImmediateResponse) GetBarcodeOk() (*Barcode, bool) {
	if o == nil {
		return nil, false
	}
	return o.Barcode.Get(), o.Barcode.IsSet()
}

// HasBarcode returns a boolean if a field has been set.
func (o *ReceiptImmediateResponse) HasBarcode() bool {
	if o != nil && o.Barcode.IsSet() {
		return true
	}

	return false
}

// SetBarcode gets a reference to the given NullableBarcode and assigns it to the Barcode field.
func (o *ReceiptImmediateResponse) SetBarcode(v Barcode) {
	o.Barcode.Set(&v)
}
// SetBarcodeNil sets the value for Barcode to be an explicit nil
func (o *ReceiptImmediateResponse) SetBarcodeNil() {
	o.Barcode.Set(nil)
}

// UnsetBarcode ensures that no value is present for Barcode, not even an explicit nil
func (o *ReceiptImmediateResponse) UnsetBarcode() {
	o.Barcode.Unset()
}

// GetMerchantReceiptId returns the MerchantReceiptId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ReceiptImmediateResponse) GetMerchantReceiptId() string {
	if o == nil || IsNil(o.MerchantReceiptId.Get()) {
		var ret string
		return ret
	}
	return *o.MerchantReceiptId.Get()
}

// GetMerchantReceiptIdOk returns a tuple with the MerchantReceiptId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReceiptImmediateResponse) GetMerchantReceiptIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MerchantReceiptId.Get(), o.MerchantReceiptId.IsSet()
}

// HasMerchantReceiptId returns a boolean if a field has been set.
func (o *ReceiptImmediateResponse) HasMerchantReceiptId() bool {
	if o != nil && o.MerchantReceiptId.IsSet() {
		return true
	}

	return false
}

// SetMerchantReceiptId gets a reference to the given NullableString and assigns it to the MerchantReceiptId field.
func (o *ReceiptImmediateResponse) SetMerchantReceiptId(v string) {
	o.MerchantReceiptId.Set(&v)
}
// SetMerchantReceiptIdNil sets the value for MerchantReceiptId to be an explicit nil
func (o *ReceiptImmediateResponse) SetMerchantReceiptIdNil() {
	o.MerchantReceiptId.Set(nil)
}

// UnsetMerchantReceiptId ensures that no value is present for MerchantReceiptId, not even an explicit nil
func (o *ReceiptImmediateResponse) UnsetMerchantReceiptId() {
	o.MerchantReceiptId.Unset()
}

// GetStoreId returns the StoreId field value
func (o *ReceiptImmediateResponse) GetStoreId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.StoreId
}

// GetStoreIdOk returns a tuple with the StoreId field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetStoreIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StoreId, true
}

// SetStoreId sets field value
func (o *ReceiptImmediateResponse) SetStoreId(v string) {
	o.StoreId = v
}

// GetOrganisationId returns the OrganisationId field value
func (o *ReceiptImmediateResponse) GetOrganisationId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OrganisationId
}

// GetOrganisationIdOk returns a tuple with the OrganisationId field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetOrganisationIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrganisationId, true
}

// SetOrganisationId sets field value
func (o *ReceiptImmediateResponse) SetOrganisationId(v string) {
	o.OrganisationId = v
}

// GetQrCodeUrl returns the QrCodeUrl field value
func (o *ReceiptImmediateResponse) GetQrCodeUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.QrCodeUrl
}

// GetQrCodeUrlOk returns a tuple with the QrCodeUrl field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetQrCodeUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.QrCodeUrl, true
}

// SetQrCodeUrl sets field value
func (o *ReceiptImmediateResponse) SetQrCodeUrl(v string) {
	o.QrCodeUrl = v
}

// GetQrToken returns the QrToken field value
func (o *ReceiptImmediateResponse) GetQrToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.QrToken
}

// GetQrTokenOk returns a tuple with the QrToken field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetQrTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.QrToken, true
}

// SetQrToken sets field value
func (o *ReceiptImmediateResponse) SetQrToken(v string) {
	o.QrToken = v
}

// GetQrExpiresAt returns the QrExpiresAt field value
// If the value is explicit nil, the zero value for time.Time will be returned
func (o *ReceiptImmediateResponse) GetQrExpiresAt() time.Time {
	if o == nil || o.QrExpiresAt.Get() == nil {
		var ret time.Time
		return ret
	}

	return *o.QrExpiresAt.Get()
}

// GetQrExpiresAtOk returns a tuple with the QrExpiresAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReceiptImmediateResponse) GetQrExpiresAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.QrExpiresAt.Get(), o.QrExpiresAt.IsSet()
}

// SetQrExpiresAt sets field value
func (o *ReceiptImmediateResponse) SetQrExpiresAt(v time.Time) {
	o.QrExpiresAt.Set(&v)
}

// GetReceiptUrl returns the ReceiptUrl field value
func (o *ReceiptImmediateResponse) GetReceiptUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ReceiptUrl
}

// GetReceiptUrlOk returns a tuple with the ReceiptUrl field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetReceiptUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ReceiptUrl, true
}

// SetReceiptUrl sets field value
func (o *ReceiptImmediateResponse) SetReceiptUrl(v string) {
	o.ReceiptUrl = v
}

// GetTransactionDate returns the TransactionDate field value
func (o *ReceiptImmediateResponse) GetTransactionDate() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.TransactionDate
}

// GetTransactionDateOk returns a tuple with the TransactionDate field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetTransactionDateOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TransactionDate, true
}

// SetTransactionDate sets field value
func (o *ReceiptImmediateResponse) SetTransactionDate(v time.Time) {
	o.TransactionDate = v
}

// GetTotalAmount returns the TotalAmount field value
func (o *ReceiptImmediateResponse) GetTotalAmount() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TotalAmount
}

// GetTotalAmountOk returns a tuple with the TotalAmount field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetTotalAmountOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TotalAmount, true
}

// SetTotalAmount sets field value
func (o *ReceiptImmediateResponse) SetTotalAmount(v string) {
	o.TotalAmount = v
}

// GetCurrency returns the Currency field value
func (o *ReceiptImmediateResponse) GetCurrency() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Currency
}

// GetCurrencyOk returns a tuple with the Currency field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetCurrencyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Currency, true
}

// SetCurrency sets field value
func (o *ReceiptImmediateResponse) SetCurrency(v string) {
	o.Currency = v
}

// GetProcessingStatus returns the ProcessingStatus field value if set, zero value otherwise.
func (o *ReceiptImmediateResponse) GetProcessingStatus() string {
	if o == nil || IsNil(o.ProcessingStatus) {
		var ret string
		return ret
	}
	return *o.ProcessingStatus
}

// GetProcessingStatusOk returns a tuple with the ProcessingStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetProcessingStatusOk() (*string, bool) {
	if o == nil || IsNil(o.ProcessingStatus) {
		return nil, false
	}
	return o.ProcessingStatus, true
}

// HasProcessingStatus returns a boolean if a field has been set.
func (o *ReceiptImmediateResponse) HasProcessingStatus() bool {
	if o != nil && !IsNil(o.ProcessingStatus) {
		return true
	}

	return false
}

// SetProcessingStatus gets a reference to the given string and assigns it to the ProcessingStatus field.
func (o *ReceiptImmediateResponse) SetProcessingStatus(v string) {
	o.ProcessingStatus = &v
}

// GetCreatedAt returns the CreatedAt field value
func (o *ReceiptImmediateResponse) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *ReceiptImmediateResponse) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *ReceiptImmediateResponse) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

func (o ReceiptImmediateResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ReceiptImmediateResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["public_token"] = o.PublicToken
	if o.Barcode.IsSet() {
		toSerialize["barcode"] = o.Barcode.Get()
	}
	if o.MerchantReceiptId.IsSet() {
		toSerialize["merchant_receipt_id"] = o.MerchantReceiptId.Get()
	}
	toSerialize["store_id"] = o.StoreId
	toSerialize["organisation_id"] = o.OrganisationId
	toSerialize["qr_code_url"] = o.QrCodeUrl
	toSerialize["qr_token"] = o.QrToken
	toSerialize["qr_expires_at"] = o.QrExpiresAt.Get()
	toSerialize["receipt_url"] = o.ReceiptUrl
	toSerialize["transaction_date"] = o.TransactionDate
	toSerialize["total_amount"] = o.TotalAmount
	toSerialize["currency"] = o.Currency
	if !IsNil(o.ProcessingStatus) {
		toSerialize["processing_status"] = o.ProcessingStatus
	}
	toSerialize["created_at"] = o.CreatedAt
	return toSerialize, nil
}

func (o *ReceiptImmediateResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"public_token",
		"store_id",
		"organisation_id",
		"qr_code_url",
		"qr_token",
		"qr_expires_at",
		"receipt_url",
		"transaction_date",
		"total_amount",
		"currency",
		"created_at",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varReceiptImmediateResponse := _ReceiptImmediateResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varReceiptImmediateResponse)

	if err != nil {
		return err
	}

	*o = ReceiptImmediateResponse(varReceiptImmediateResponse)

	return err
}

type NullableReceiptImmediateResponse struct {
	value *ReceiptImmediateResponse
	isSet bool
}

func (v NullableReceiptImmediateResponse) Get() *ReceiptImmediateResponse {
	return v.value
}

func (v *NullableReceiptImmediateResponse) Set(val *ReceiptImmediateResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableReceiptImmediateResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableReceiptImmediateResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReceiptImmediateResponse(val *ReceiptImmediateResponse) *NullableReceiptImmediateResponse {
	return &NullableReceiptImmediateResponse{value: val, isSet: true}
}

func (v NullableReceiptImmediateResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReceiptImmediateResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


